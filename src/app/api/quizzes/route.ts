...
export async function GET() {
  try {
    const { data, error } = await supabaseAdmin.from('quizzes').select('*').order('created_at', { ascending: false })
    if (error) {
      if (String(error.message).includes('created_at')) {
        const { data: d2, error: e2 } = await supabaseAdmin.from('quizzes').select('*')
