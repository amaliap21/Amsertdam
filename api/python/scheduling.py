...
EFFECTIVE_W_STRESS_PENALTY = 0.10
...
def _bucket_priority_score(task: dict):
...
def _default_stress_penalty(task: dict) -> float:
    stress_level = _safe_float(task.get("stress_level", 3), 3)
    return _normalize(stress_level, 1, 5)
...
def _derive_stress_level(task: dict, effort_label: str, estimated_hours: float) -> float:
    if task.get("stress_level") is not None:
        return _safe_float(task.get("stress_level"), 3)

    if effort_label == "high" or estimated_hours > 5:
        return 4.0
    if effort_label == "medium" or estimated_hours > 2:
        return 3.0
    return 2.0
...
def _priority_analysis_task_payload(
    ...
    stress_level = _derive_stress_level(task, effort_label, estimated_hours)
...
def _analysis_summary(results: list[dict]) -> dict:
    return {
        "total": len(results),
...
def _synthetic_analysis(task: dict, current_grade: float, passing_grade: float) -> dict:
...
def _run_priority_analysis_pipeline(canonical_tasks: list[dict], analysis_method: str, current_grade: float, passing_grade: float) -> dict:
    payload = {
    ...
    if module is not None and hasattr(module, "analyze_batch"):
        try:
            ...
            if isinstance(analyzed, dict):
                ...
                report["summary"] = _analysis_summary(analyzed)
                report["source"] = "priority_analysis"
        except Exception as exc:
            report["error"] = str(exc)

    if not report.get("tasks"):
        report["tasks"] = [_synthetic_analysis(task, current_grade, passing_grade) for task in canonical_tasks]
        report["summary"] = _analysis_summary(report["tasks"])
...
def _merge_analysis_results(canonical_tasks: list[dict], analysis_report: dict, current_grade: float, passing_grade: float) -> list[dict]:
    ...
    for task in canonical_tasks:
        ...
        if analysis is None:
            analysis = _synthetic_analysis(task, current_grade, passing_grade)
        merged.append({
            **task,
            "analysis": analysis,
        })
...
def _effective_score(task: dict, current_grade: float, passing_grade: float) -> float:
    """
    Hybrid effective score optimized for scheduling.
    Prioritizes TOPSIS/composite outputs when available, with robust fallbacks.
    """
    ...
    stress_penalty = _clip01(_safe_float(breakdown.get("stress_penalty"), _default_stress_penalty(task)))
    ...
    penalty = (
        EFFECTIVE_W_EFFORT_PENALTY * effort_penalty
        + EFFECTIVE_W_STRESS_PENALTY * stress_penalty
    )
...
def recommend_priorities(data: dict) -> dict:
    ...
    analysis_report = _run_priority_analysis_pipeline(canonical_tasks, analysis_method, current_grade, passing_grade)
    ...
    for task in merged_tasks:
        score = _effective_score(task, current_grade, passing_grade)
        scored.append({
            **task,
...
def _print_gantt_chart(schedule: list, days_span: int = 14):
    """Print a simple ASCII gantt chart of the schedule."""
    if not schedule:
...
def _print_ranked_tasks(ranked_tasks: list):
    """Print ranked tasks in a clean table format."""
    if not ranked_tasks:
...
if __name__ == "__main__":
    ...
    _print_ranked_tasks(ranked)
    
    # Print gantt chart
    schedule = result.get("schedule", [])
    _print_gantt_chart(schedule, days_span=14)